@extends('layouts.main')
@push('title')
<title>Login</title>
@endpush
@section('content')

<div class="container-fluid bg-light p-5">
    <h1 class="text-center"><i class="fa-solid fa-user"></i>User Login</h1>
</div>
<!-- Cart List Section -->

<section>
    <div class="container my-5">
        <div class="row">
            <div class="col-lg-10">
                <div class="row">
                    <div class="col-lg-6">
                        <div>
                            <img src="{{asset('assets/images/register.jpg')}}" class="rounded-3 img-fluid">
                        </div>
                    </div>

                    <div class="col-lg-6">
                        <form>
                            <div class="mb-3">
                                <div class="form-text mb-2">Please Enter Your Mobile Number </div>
                                <input type="number" class="form-control form-control-lg"  placeholder="+91">

                            </div>
                           
                            <a href="{{url('/login1')}}" type="btn" class="btn text-light form-control form-control-lg theme-orange-btn">Request OTP</a>
                            <div class="text-center py-5 my-5"> Don't Have an account? <a href="{{url('register')}}" class="text-decoration-none">Signup</a></div>
                        </form>

                        <div>

                        </div>
                    </div>

                </div>

            </div>

        </div>
    </div>
</section>






@endsection